...
\textbackslash caption\{Contrast matching functions. Dotted and dashed lines are predictions of canonical summation models with a linear exponent (dotted) or an infinite exponent (dashed). Error bars indicate the standard error across participants (N=10), and are constrained along radial lines converging at the origin. Note that, for the 48\% match, the data point on the x axis falls higher than 100\% contrast. This is because some individual results were interpolated above 100\%, therefore skewing the results slighlty above that value. The effects are negligible.\}\label{fig:matchingdata}
