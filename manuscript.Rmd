...
author: |
 | Federico G. Segala$^1$, Aurelio Bruno$^2$, Joel T. Martin$^1$, Myat T. Aung$^1$, Alex R. Wade$^1$
 | & Daniel H. Baker$^1$
output:
...
Address for correspondence: Federico G. Segala, [fgs502\@york.ac.uk](fgs502@york.ac.uk)
...
The pupillary light reflex is an automatic constriction of the iris sphincter muscles in response to increases in light levels, which causes the pupil to shrink [@McDougal2008]. There is a clear binocular component to this reflex, as stimulation of one eye still causes constriction of the other eye's pupil (termed the consensual response; @Wyatt1981). Importantly, the neuroanatomical pathway involved completely bypasses the canonical cortical pathway (retina to V1), instead involving a network of subcortical nuclei, including the Pretectal Olivary nucleus, Superior Cervical ganglion, and Edinger-Westphal nucleus [@Angee2021; @Mathot2018; @McDougal2008; @Wang2015]. To account for the consensual response, these brain regions must combine information from the left and right eyes [@Doesschate1967], yet the computation that achieves this is unclear. The pupil response can be modulated by periodic changes in luminance, and is temporally low-pass [@Barrionuevo2014; @Spitschan2014], most likely due to the mechanical limitations of the iris sphincter and dilator muscles [@Privitera2006]. 
...
for (cond in 1:3){
  points(contrastsdB,abs(cleanmeansP[cond,]), pch = 20+cond, col=colpal[cond], bg=colpal[cond], cex=1.6, lwd=3)   # draw the data points themselves
}  
cond <- 6
  points(contrastsdB,abs(cleanmeansP[cond,]), pch = 25, col=colpal[cond], bg=colpal[cond], cex=1.6, lwd=3) 

legend(21,0.03,c('Monocular','Binocular','Dichoptic','Dichoptic cross'),pch=c(21:23,25),pt.bg=colpal[c(1:3,6)],lwd=3,lty=1,col=colpal[c(1:3,6)],pt.lwd=3,pt.cex=1.6,box.lwd=2)
...
for (cond in 1:3){
  points(contrastsdB,abs(cleanmeansEav[cond,]), pch = 20+cond, col=colpal[cond], bg=colpal[cond], cex=1.6, lwd=3)   # draw the data points themselves
}  
cond <- 6
  points(contrastsdB,abs(cleanmeansEav[cond,]), pch = 25, col=colpal[cond], bg=colpal[cond], cex=1.6, lwd=3)   # draw the data points 
...
for (cond in 1:3){
  points(contrastsdB,abs(cleanmeansE2av[cond,]), pch = 20+cond, col=colpal[cond], bg=colpal[cond], cex=1.6, lwd=3)   # draw the data points themselves
}  
cond <- 6
  points(contrastsdB,abs(cleanmeansE2av[cond,]), pch = 25, col=colpal[cond], bg=colpal[cond], cex=1.6, lwd=3)  
...
legend(62,95,c('48% match','24% match'),col=colpal[2:1],lwd=3,pch=16,pt.cex=2,box.lwd=2)
...
dev.off()
  ...
  pdf(paste0(figdir,"modelfigure.pdf"), bg="transparent", height = 5, width = 25)
  par(mar=c(0.1,0.1,0.1,0.1))
  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(0,1), ylim=c(0,1))

  grid.picture(p1,x=1/10,y=0.5,width=0.2,height=1)
  grid.picture(p2,x=3/10,y=0.5,width=0.2,height=1)
  grid.picture(p3,x=5/10,y=0.5,width=0.2,height=1)
  grid.picture(p4,x=7/10,y=0.5,width=0.2,height=1)
  grid.picture(p5,x=9/10,y=0.5,width=0.2,height=1)

  dev.off()
  
  file.remove(c('CRF1e.ps','CRF2e.ps','CRF1p.ps','matchingmodel.ps','posteriors.ps'))
  file.remove(c('CRF1e.ps.xml','CRF2e.ps.xml','CRF1p.ps.xml','matchingmodel.ps.xml','posteriors.ps.xml'))
...
colnames(tabledata) <- c('Data set','Z','n','w','Rmax')
...
```{r appendixfig1, fig.cap="Summary of pupillometry results for N = 12 participants, for peripheral stimulation. See Figure 1 for a description of each panel.", fig.align="center", echo=FALSE}
...
```{r appendixfig2, fig.cap="Summary of steady-state EEG results for N = 12 participants, for peripheral stimulation. See Figure 2 for a description of each panel.", fig.align="center", echo=FALSE}
