...
for (participant in 1:30){
         ...
         if (!file.exists(paste0(datadir,'P',100+participant,'summary.RData'))){
          ...
          if (!file.exists(paste0(datadir,'headerfile.csv'))){
           ...
           osfproject <- osf_retrieve_node(osfnode)
          osffilesP <- osf_ls_files(osfproject,n_max=300)}
          fid <- which(osffilesP$name==paste0('P',100+participant,'headerfile.csv'))
          osf_download(osffilesP[fid,],datadir,progress=TRUE)
          } 
...
filt <- butter(3,0.083)
filtwaveP <- filter(filt, meanwavesP)
filtU <- filter(filt,meanwavesPCI[1,])
filtL <- filter(filt,meanwavesPCI[2,])
...
filtwaveE <- filter(filt, meanwavesE)
filtU <- filter(filt,meanwavesECI[1,])
filtL <- filter(filt,meanwavesECI[2,])
polygon(times[c(1:length(times),length(times):1)],c(filtL,filtU[length(filtU):1]),col=colpal[2],border=NA)
...
# download and process data if this has not been done already for this participant
    if (!file.exists(paste0(datadir,'P',150+participant,'summary.RData'))){   
      ...
      if (length(d)==0){
        ...
        # then unzip the tar file
        untar(paste0(rawdir,'P',150+participant,'.tar'),exdir=paste0(rawdir,'P',100+participant))
...
knitr::include_graphics('local/Figures/TFdata.pdf')
...
knitr::include_graphics('local/Figures/matchingdata.pdf')
...
numSavedSteps <- 20000    # total number of MCMC steps
...
plotlims <- c(-48,12,0,0.3) 
ticklocsx <- seq(-48,12,12)    
ticklocsy <- seq(0,0.3,0.1)    
...
legend(-48,0.3,c('Prior','Pupillometry','EEG 1F','EEG 2F','Matching'),col=c('black',colpal[3:6]),lwd=c(1,3,3,3,3),lty=1, box.lwd=2, bg='white')

text(9,0.28,'(e)',cex=1.25,adj=0.5)
...
text(17,0.038,'(a)',adj=0.5,cex=2.5)
...
dev.off()
  ...
  pdf(paste0(figdir,"modelfigure.pdf"), bg="transparent", height = 10, width = 20)
  par(mar=c(0.1,0.1,0.1,0.1))
  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(0,1), ylim=c(0,1))
  

  grid.picture(p1,x=0.125,y=0.75,width=0.25,height=1)
  grid.picture(p2,x=0.375,y=0.75,width=0.25,height=1)
  grid.picture(p3,x=0.125,y=0.25,width=0.25,height=1)
  grid.picture(p4,x=0.375,y=0.25,width=0.25,height=1)

  grid.picture(p5,x=0.75,y=0.5,width=0.5,height=1)
...
knitr::include_graphics('local/Figures/modelfigure.pdf')
